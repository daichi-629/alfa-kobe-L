<template>
  <transition name="change-image" mode="out-in">
    <img
      :key="this.imageObjects[this.number].index"
      @click="toggle"
      :src="this.imageObjects[this.number].image"
      :alt="this.imageObjects[this.number].alt"
    />
  </transition>
</template>
...
<style>
.change-image-enter-active,
.change-image-leave-active {
  transition: opacity 0.1s ease;
}
.change-image-enter-from,
.change-image-leave-to {
  opacity: 0;
}
</style>
